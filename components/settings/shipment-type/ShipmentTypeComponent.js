...
const ShipmentTypeComponent = ({ shipmentTypes }) => {
  const [shipmentType, setShipmentType] = useState('');
  //   const [shipmentTypes, setShipmentTypes] = useState([]);

  const addShipmentType = async () => {
    const { data } = await axios.post(`/api/addShipmentType`, {
      shipmentType,
    });
  };
  ...
  //     const { data } = await axios.get(`http://localhost:8000/api/getAllShipmentTypes`);
